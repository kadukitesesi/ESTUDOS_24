...
function exibirPergunta(pergunta, index) { // Função que exibe a pergunta e retornar a resposta do usuário
  const respostaUsuario = window.prompt(`\n${index + 1}: ${pergunta.pergunta}`);
...
function exibirResultado(pontuacaoFinal, nomeJogador) { // Função que retornar uma mensagem com base na pontuação do jogador
  ...
  if (pontuacaoFinal <= 3) {
    alert ( 'OH NÃO! Tente mais uma vez.');
  } else if (pontuacaoFinal <= 6) {
    alert('BOM TRABALHO! Pratique um pouco mais.');
  } else if (pontuacaoFinal <= 9) {
    alert( 'MUITO BOM! Você acertou a maioria.');
  } else {
    alert('EXCELENTE! Você é um verdadeiro expert.');
  }

  alert(`\nJogador(a): ${nomeJogador}
  Pontuação final: ${pontuacaoFinal} acertos ${mensagem}`)
}
...
function iniciarQuiz() {
  alert(`--------QUIZ DE FATOS HISTÓRICOS--------
